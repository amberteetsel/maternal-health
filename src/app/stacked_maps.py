...
def generate_stacked_us_maps(df, measure_name, color_scale, title_text):
    ...
    """
    # filter for the requested measure
    measure_df = df[df['Measure'] == measure_name].copy()
    
    # convert missing data flags (like 9999) to NaN so Plotly treats them as empty
    measure_df['Value'] = measure_df['Value'].replace(9999, np.nan)
    
    # separate the data by target years
    df_2018 = measure_df[measure_df['Year'] == 2018]
    df_2023 = measure_df[measure_df['Year'] == 2023]
    
    # find the common min/max range across both years to ensure a synchronized color bar
    ...
    # build subplots stacked vertically (2 Rows, 1 Column)
    fig = make_subplots(
        rows=2, cols=1,
        row_heights=[0.5, 0.5],
    ...
    # config to ensure missing states show up as light gray land
    geo_config = dict(
        scope='usa',
    ...
    # layouts and shared map geo parameters
    fig.update_layout(
